...
def analyze_with_gpt(transcript, call_info=None):
    """
    Analyze call transcript using OpenAI GPT-4 with external prompts.
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            max_tokens=1500,  # Увеличили лимит для более детального анализа
            temperature=0.3   # Снизили температуру для более точного анализа
        )
...
def main():
    ...
    for i, call in enumerate(incoming_calls_with_recordings):
        ...
        # 🔒 EARLY SAVE: Mark call as being processed to prevent duplicates
        print(f"🔒 Marking call {call_uuid} as processing to prevent duplicates...")
